...
add_action( 'vc_load_default_templates_action','corporate_template_for_vc' ); // Hook in
 
function corporate_template_for_vc() {
    $data               	= array(); 
    $data['name']       	= __( 'Corporate Template (Pro Only)', 'tavc' );
    $data['weight']     	= 0; 
    $data['image_path'] 	= plugins_url( 'assets/img/templates/e.png', __FILE__ );//Thumbnail should have this dimensions: 114x154px
    $data['custom_class'] 	= 'corporate_template_for_vc_custom_template';
